...
export function Sidebar({ role }: SidebarProps) {
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-64 bg-white border-r border-slate-200 z-40 flex flex-col">
      {/* Logo */}
          ...
          {items.map((item) => {
            ...
            return (
              <li key={item.href}>
                ...
                >
                  <item.icon
                    className={`w-5 h-5 ${isActive ? "text-sky-600" : ""}`}
                  />
                  {item.label}
                  {isActive && (
                    <div className="ml-auto w-1.5 h-1.5 rounded-full bg-sky-600" />
                  )}
                </Link>
              </li>
            );
