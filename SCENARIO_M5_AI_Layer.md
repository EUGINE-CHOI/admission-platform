...
|----------|-------|------|
| ~~OpenAI~~ | ~~GPT-4~~ | 변경됨 |
| **Google Gemini** | **gemini-2.0-flash** | ✅ 활성 |
...
| `SUBJECT_ADVICE` | 교과 조언 / 빠른 조언 / 종합 분석 |
| `READING_GUIDE` | 독서 추천/가이드 |
| `ACTION_PLAN` | 액션 플랜 |
| `QUICK_ADVICE` | 빠른 맞춤 조언 (신규) |
| `COMPREHENSIVE_ANALYSIS` | 종합 분석 (신규) |
| `SCHOOL_RECOMMENDATIONS` | AI 학교 추천 (신규) |
...
## 🆕 WP5.6 — AI 고도화 기능 (신규 추가)

### Scenario WP5.6-1: 빠른 조언 (Quick Advice) 요청 성공

**Given**: 학생이 로그인되어 있다.

**When**: 특정 주제(예: "과학고 입시 준비")와 함께 빠른 조언을 요청한다.

**Then**: 201 Created, AI가 다음을 포함한 맞춤형 조언을 반환한다:
- 현재 상태 분석
- 구체적 조언 3가지 (실행 가능한 액션 포함)
- 이번 주 목표 3가지
- 격려 메시지
- 다음 단계 안내
...
### Scenario WP5.6-2: 종합 분석 (Comprehensive Analysis) 성공

**Given**: 학생의 성적, 활동, 목표학교 데이터가 존재한다.

**When**: 종합 분석 API를 호출한다.

**Then**: 201 Created, AI가 다음을 포함한 종합 분석을 반환한다:
- 종합 평가 점수 (0-100) 및 등급 (A~F)
- 학업 분석 (강점/약점/추세)
- 비교과 활동 분석 (다양성/심화도)
- 목표 학교별 적합도 분석 (확률 포함)
- 우선순위별 실천 과제
...
**선행 Scenario**: WP2.1-1, WP4.0-1, WP5.1-1

---

### Scenario WP5.6-3: AI 학교 추천 성공

**Given**: 학생의 성적과 활동 데이터가 존재한다.

**When**: 선호 지역/학교유형과 함께 학교 추천 API를 호출한다.

**Then**: 201 Created, AI가 다음을 포함한 학교 추천을 반환한다:
- 추천 학교 5개 (적합도 점수, 추천 이유, 합격 요건)
- 대안 학교 목록
- 전반적인 입시 전략 조언

**선행 Scenario**: WP2.1-1, WP3.1-1, WP5.1-1

---

### Scenario WP5.6-4: 데이터 부족 시에도 기본 조언 제공

**Given**: 학생의 성적/활동 데이터가 없다.

**When**: 빠른 조언을 요청한다.

**Then**: 201 Created, AI가 데이터 입력을 권장하는 내용과 함께 일반적인 입시 준비 조언을 반환한다.
...
> AI 호출의 핵심 인프라. 모든 AI 에이전트가 이 Orchestrator를 통해 Google Gemini API를 호출함.
...
|----|--------|----------|------|------|
| WP5.1 | GET | `/ai/health` | AI 서비스 상태 확인 (Gemini) | PUBLIC |
...
| **WP5.6** | **POST** | `/ai/advice/quick` | **빠른 맞춤 조언 (신규)** | STUDENT |
| **WP5.6** | **GET** | `/ai/advice/quick` | **빠른 조언 (GET)** | STUDENT |
| **WP5.6** | **POST** | `/ai/analysis/comprehensive` | **종합 분석 (신규)** | STUDENT |
| **WP5.6** | **POST** | `/ai/recommend/school` | **AI 학교 추천 (신규)** | STUDENT |
